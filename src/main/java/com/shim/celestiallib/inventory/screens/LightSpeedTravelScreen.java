...
//        super.render(poseStack, mouseX, mouseY, delta);
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int p_97750_) {
        ...
        if (isHovering(x - this.leftPos + 9, y - this.topPos + 107, 210, 100, mouseX, mouseY)) { //mouse clicked in galaxy map
    ...
    public void renderGalaxyMap(PoseStack poseStack, int x, int y) {
        ...
        blit(poseStack, x + 9, y + 107, xPos, yPos, 210, 100, size, size);
    ...
    public void renderPlanet(PoseStack poseStack, Planet planet, int x, int y, int galaxySize, int galaxyX, int galaxyY) {
        ...
        int xLoc = x + 9 + 105 - (size / 2) + xMovement + (int) (planetLoc.x() / scale);
...
//        CelestialLib.LOGGER.debug("yLoc: " + yLoc + ", galaxyY: " + galaxyY + ", yDrag: " + yDrag + ", yMovement: " + yMovement);
        ...
        if (xLoc < x + 9) {
            xOffset = Mth.clamp(x + 9 - xLoc, 0, size);
            xStarting = xOffset;
        }
        if (xLoc > x + 219 - size) {
        ...
        xLoc = Mth.clamp(xLoc, x + 9, x + 219);

//        if (xLoc < 9 - size || xLoc > 219 + size || yLoc < 107 || yLoc > 207)
//            return;
