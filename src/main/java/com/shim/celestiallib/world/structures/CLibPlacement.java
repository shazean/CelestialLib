...
public class CLibPlacement {

    public static Optional<PieceGenerator<CLibConfiguration>> addPieces(PieceGeneratorSupplier.Context<CLibConfiguration> p_210285_, CLibPlacement.PieceFactory p_210286_, BlockPos p_210287_, boolean p_210288_, boolean p_210289_) {
        ...
        CLibConfiguration jigsawconfiguration = p_210285_.config();
            ...
            if (!predicate.test(chunkgenerator.getNoiseBiome(QuartPos.fromBlock(i), QuartPos.fromBlock(k), QuartPos.fromBlock(j)))) {
                ...
                return Optional.of((p_210282_, p_210283_) -> {
                    ...
                    if (jigsawconfiguration.maxDepth() > 0) {
                        ...
                        CLibPlacement.Placer jigsawplacement$placer = new CLibPlacement.Placer(registry, jigsawconfiguration.maxDepth(), p_210286_, chunkgenerator, structuremanager, list, worldgenrandom);
                        jigsawplacement$placer.placing.addLast(new CLibPlacement.PieceState(poolelementstructurepiece, new MutableObject<>(Shapes.join(Shapes.create(aabb), Shapes.create(AABB.of(boundingbox)), BooleanOp.ONLY_FIRST)), 0));

                        while(!jigsawplacement$placer.placing.isEmpty()) {
                            CLibPlacement.PieceState jigsawplacement$piecestate = jigsawplacement$placer.placing.removeFirst();
    ...
    public static void addPieces(RegistryAccess p_210291_, PoolElementStructurePiece p_210292_, int p_210293_, CLibPlacement.PieceFactory p_210294_, ChunkGenerator p_210295_, StructureManager p_210296_, List<? super PoolElementStructurePiece> p_210297_, Random p_210298_, LevelHeightAccessor p_210299_) {
        Registry<StructureTemplatePool> registry = p_210291_.registryOrThrow(Registry.TEMPLATE_POOL_REGISTRY);
        CLibPlacement.Placer jigsawplacement$placer = new CLibPlacement.Placer(registry, p_210293_, p_210294_, p_210295_, p_210296_, p_210297_, p_210298_);
        jigsawplacement$placer.placing.addLast(new CLibPlacement.PieceState(p_210292_, new MutableObject<>(Shapes.INFINITY), 0));
    ...
    static final class Placer {
        ...
        private final CLibPlacement.PieceFactory factory;
        ...
        Placer(Registry<StructureTemplatePool> p_210323_, int p_210324_, CLibPlacement.PieceFactory p_210325_, ChunkGenerator p_210326_, StructureManager p_210327_, List<? super PoolElementStructurePiece> p_210328_, Random p_210329_) {
            ...
            for(StructureTemplate.StructureBlockInfo structuretemplate$structureblockinfo : structurepoolelement.getShuffledJigsawBlocks(this.structureManager, blockpos, rotation, this.random)) {
                ...
                if (optional.isPresent() && (optional.get().size() != 0 || Objects.equals(resourcelocation, Pools.EMPTY.location()))) {
                    ...
                    if (optional1.isPresent() && (optional1.get().size() != 0 || Objects.equals(resourcelocation1, Pools.EMPTY.location()))) {
                                    ...
                                    if (JigsawBlock.canAttach(structuretemplate$structureblockinfo, structuretemplate$structureblockinfo1)) {
                                        ...
                                        if (!Shapes.joinIsNotEmpty(mutableobject1.getValue(), Shapes.create(AABB.of(boundingbox3).deflate(0.25D)), BooleanOp.ONLY_SECOND)) {
                                            ...
                                            if (p_210336_ + 1 <= this.maxDepth) {
                                                this.placing.addLast(new CLibPlacement.PieceState(poolelementstructurepiece, mutableobject1, p_210336_ + 1));
                                            }
                                            continue label139;
                                        }
                                    }
                        ...
                        CelestialLib.LOGGER.warn("Empty or non-existent fallback pool: {}", (Object)resourcelocation1);
                    }
                } else {
                    CelestialLib.LOGGER.warn("Empty or non-existent pool: {}", (Object)resourcelocation);
                }
            }
